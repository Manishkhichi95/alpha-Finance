      ...
      try {
        ...
        const reserveDataPromises = getReserveData[0].map(async (element: any) => {
          ...
          return {
            ...
            deposit: ((Number(deposit)) / (Number(1 + '0'.repeat(Number(decimals)))) * (Number(getAssetPrice) / decimalVal)).toFixed(2),
            totalSupply: (Number(deposit) * (Number(getAssetPrice) / decimalVal) / (Number(1 + '0'.repeat(Number(decimals))))).toFixed(2),
