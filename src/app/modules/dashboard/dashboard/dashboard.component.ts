  ...
  async getUserReservesData() {
    if (this.networkName == 'Mumbai Testnet') {
      ...
      depositedAssetContract[0].forEach(async (res: any) => {
        ...
        new this.web3.eth.Contract(this.tokenContractsABI, this.depositedAsset.address);
        ...
        if (res.scaledATokenBalance != 0) {
          ...
          const depositedBalance = res.scaledATokenBalance;
          this.depositedAsset.balance = (Number(depositedBalance) / Math.pow(10, 18)).toFixed(2);
          console.log(this.depositedAsset.balance, this.balanceAsset)
        }
  ...
  async setCurrentchainId(chainId: string) {
    ...
    if (this.networkName == 'Mumbai Testnet') {
      ...
      depositedAssetContract[0].forEach(async (res: any) => {
        ...
        this.depositedAsset.decimals = await tokenContracts.methods.decimals().call();
        this.depositedAsset.totalSupply = await tokenContracts.methods.totalSupply().call();
        const balance = res.scaledATokenBalance;
        this.depositedAsset.balance = (Number(balance) / Math.pow(10, 18)).toFixed(3);
        console.log('depositedASSET', this.depositedAsset)
      });
    }
  }
